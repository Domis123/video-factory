...
- **Segment analyzer deep rewrite shipped (2026-04-18).** Full re-segmentation complete: 191 assets, 611→903 segments, exercise max 12s, subject appearance 100%, movement phase tags.
- **Prompt fixes shipped (2026-04-18).** Hook duration floors, CD visual descriptions, curator prep-clip rejection. Curator scores improved 4/10→9/10.
- **Architecture pivot identified (2026-04-18).** CD designs videos for exercises it can't verify exist → clips don't match overlay text. Fix: library-aware CD + post-selection copywriting. See `HANDOFF_PHASE3_ARCHITECTURE_PIVOT.md`.
- **CTA hotfix deployed** (was pending, now merged and live).
- **Data (2026-04-18):** 191 nordpilates assets, 903 asset_segments (freshly re-segmented, avg exercise 6.2s, subject appearance 100%), 15 music_tracks (no calm/ambient), 5 brand_configs, ~10 jobs.
