...
## Architecture Rules (42 total, MUST follow)
...
38. **LLMs confabulate structure to match prompt expectations on out-of-distribution inputs.** When a prompt encodes strong structural expectations, LLMs — especially older or smaller ones — will hallucinate that structure on inputs that don't contain it. The failure is insidious under `responseSchema`: output is Zod-clean, lands in production looking like real data, and is only catchable by manual spot-check against ground truth.

    This manifests in at least two distinct patterns:

    **Pattern A — Fabricating content that isn't there.** The model invents data that doesn't exist in the input to satisfy the prompt's implied structure. Example (W0b.2, 2026-04-20): `gemini-2.5-pro` analyzing a 5.5s talking-head Pilates clip returned 13 structured segments (bird-dog, russian twists, side plank, etc.) that weren't in the video. The prompt asked for "exercise segments in this Pilates clip"; the model produced what the prompt expected, not what the video contained.

    **Pattern B — Fabricating structure past edges of real data.** The model extrapolates domain-appropriate continuations beyond the actual content. Example (W0d Phase C, 2026-04-21): `gemini-3.1-pro-preview` at temperature 0.2 reliably emitted Pass 1 segment boundaries extending 40–80 seconds past actual video end on specific parents:
    - 134s parent → boundaries extending to 215s (8 fully fabricated "continuing glute-bridge reps")
    - 91s parent → boundaries extending to 132s (4 fully fabricated "left-leg after right-leg" symmetric continuations)
    - 64s parent → boundaries extending to 104s (fabricated "cycle 8" extending a rep sequence)

    Pattern B is especially dangerous because Pass 2 then dutifully tries to analyze the fabricated regions and either returns garbage metadata for content that doesn't exist, or errors out (Gemini 500/503/undici timeout depending on rejection path) in ways that look like transient API issues.

    **Mitigations (both patterns):**
    - Include explicit escape paths in prompts: "If the input does not contain [expected content], return an empty array / use the `unusable` enum / flag it." Give the model somewhere legitimate to land non-matching inputs.
    - Inject input metadata as hard constraints in prompts (Pattern B specific). For video analysis: "PARENT DURATION: {duration_s}s. Segment boundaries MUST NOT exceed this value. Do NOT extrapolate or complete implied sequences that extend past the end of the video."
    - Include specific anti-pattern examples for known confabulation modes. Generic "don't hallucinate" is weak; "do NOT invent left-leg segments after right-leg segments unless visible in footage" is strong.
    - Add consumer-side defensive validation. For Pattern B: after parsing, clamp/drop values that exceed known input bounds (e.g., boundaries past parent duration). Log warnings when the clamp fires so confabulation rate is monitorable.
    - Deliberately test prompts on edge inputs before shipping: short clips, wrong-type clips, empty-content clips.
    - Spot-check at least one sample against ground truth (watch the video, read the document) before trusting aggregate metrics. Zod validation does not validate truthfulness.
    - Prefer stronger instruction-followers when the task is structured and the input distribution is wide.
39. **Zod refines that encode soft rules as hard constraints fail on legitimate edge cases.** Zod `.refine()` is powerful for cross-field validation. It's also a common source of false-positive failures when a "general rule" is encoded as a blanket constraint that doesn't hold in every case. The pattern: a field has a typical valid range (say, "segments longer than 1.5s") and a `.refine()` enforces that as a hard constraint across all rows. Later, a legitimate edge case surfaces (some segment types can legitimately be shorter) and the refine starts rejecting valid data. **Observed instances:** (1) W0b.1, 2026-04-20 — `audio.transcript_snippet` refined as "must not be null when `has_speech=true`." But there's a legitimate case where speech is present but entirely unintelligible — the correct representation is `has_speech=false, transcript_snippet=null`, which the refine didn't exempt. Fix: add explicit escape path in the prompt rather than loosening the schema. (2) W0d Phase B, 2026-04-21 — `recommended_duration_s` refined as "must be >= 1.5 for non-unusable segments." But setup/transition/cooldown types can legitimately have sub-1.5s useful windows. Fix: scope the floor to content-bearing types only (`exercise`, `hold`, `talking-head`, `b-roll`), exempt the connective types. **Mitigations:** before adding a `.refine()`, ask "is this constraint a real invariant of the domain, or is it a typical-case heuristic?" — if the latter, scope it narrower or move it to a prompt-level guideline rather than a schema rejection; prefer per-case refines over blanket refines (a refine that applies based on `segment_type` is more defensible than one that applies to every row); when a refine fires in production on real data, the first question is "is the data wrong or is the refine wrong?" — don't assume the data is wrong without checking ground truth; document rationale inline at the refine site. **When to use Zod refines anyway:** genuine invariants that must hold for downstream code to function (e.g., `start_s < end_s` is a real invariant). Floor constraints that are about "what's typically useful" belong in prompts or policy layers, not Zod.

**40. Creative variance lives in persona + prompts, not in segment taxonomy.** Downstream quality problems (outputs feeling generic, AI-produced, or missing a desired creative register) are debugged by changing upstream creative direction — brand persona content, Planner prompt, Copywriter prompt, form taxonomy definitions — NOT by expanding the segment_type enum or adding new structural labels to Part A analyzer output. The segment taxonomy is the ingredient list; the persona + planner + copywriter are the recipe book. **Observed instance (W1 Gate B, 2026-04-21):** Domis surfaced a concern that the 8-type segment taxonomy (setup/exercise/transition/hold/cooldown/talking-head/b-roll/unusable) is fitness-shaped and over-indexes on exercise-reel content. The response is NOT to add `food-prep`, `lifestyle-ambient`, `product-focus` etc. as new segment_types; it IS to encode content variance in the brand persona's form×posture allowlist and the Planner's form_id commitment. **Why:** (1) Part A segment re-analysis is expensive — W0d was a week of work to backfill 720 segments, and creative problems should not re-trigger that cost; (2) Content forms are brand-specific while segment types should be content-agnostic — mixing them couples layers that should be orthogonal; (3) Expanding the segment enum creates asymmetric libraries across brands unless ALL brands re-analyze, which is even more expensive. **Exception:** if evidence surfaces that a fundamental content-type is structurally absent from the analyzer's ontology in a way that genuinely blocks retrieval (e.g., the library contains clear "product-beauty-shot" content that consistently gets mis-bucketed into `b-roll` or `unusable` and downstream retrieval suffers measurably), segment taxonomy expansion may be justified — but the bar is measurable retrieval failure across brands, not aesthetic dissatisfaction with a specific brand's output variety.

**41. Forms and aesthetic postures are orthogonal axes; keep them separately extensible.** A video's creative identity has two dimensions: **FORM** (structural shape — slot count, pacing profile, segment role distribution, hook mechanism) and **AESTHETIC POSTURE** (tonal/visual framing — color treatment, overlay voice, music lean, subject-continuity preference). Forms are content-agnostic (any brand could theoretically use any form); postures are brand-specific (nordpilates does not use raw-handheld-first-person). Brand persona restricts posture mix; Planner commits to form per video; Director + Copywriter execute form within posture constraint. **Why:** (1) New brands onboard by restricting posture, not by defining new forms — "one pipeline, 30 brands" is only viable if form is brand-agnostic; (2) New trends add a form OR a posture, not both — keeping them orthogonal prevents the combinatorial explosion of a single-enum approach (form-posture pairs as enum entries); (3) The Planner's job (committing to form + hook) is cleaner when separated from the brand persona's job (restricting posture); (4) Form×Posture allowlist as a per-brand grid is a legible representation that non-engineers can edit. **Canonical reference:** `docs/w2-content-form-taxonomy.md` defines the v1 form enum (16 entries) and posture enum (5 entries, P6 deferred to W10). Planner output schema includes `form_id: FormId` and `posture: AestheticPosture`; posture is passed through from brand persona, NOT chosen by Planner. **Anti-pattern:** collapsing to a single `archetype` enum with entries like "soft-pastel-microtutorial" and "raw-handheld-microtutorial" — loses the axis separation, couples form variance to posture variance, forces re-enumeration when brands expand. If future design pressure pushes toward collapsing the axes, first check whether the collapse is solving a real problem (Planner confusion? downstream schema complexity?) or whether it's premature simplification that loses expansion capacity. **Exception:** single-axis may be justified for brands with exactly one allowed posture, where form×posture reduces to just form — but persist the two-axis representation in code even if the brand's grid has one active posture column, so onboarding a second posture later is a config edit not a code edit.

**42. Mid-stream tuning iterations follow a single-gate protocol.** When a shipped workstream surfaces a prompt-tuning concern that is (a) schema-additive (no new fields, no renamed fields, no changed field semantics), (b) code-path-unchanged (no new functions, no new control flow, no new dependencies), and (c) validated by existing test scripts, the iteration uses a **single-gate brief** — Gate A = Commit A push + smoke + merge, combined — instead of the standard two-gate structure. Required brief content for single-gate workstreams: explicit "single-gate justified because…" paragraph referencing the three criteria (schema-additive, code-path-unchanged, existing tests validate); rollback section covering what's reversible if merged iteration regresses (typically: `git revert` of the merge commit restores prior prompts; zero code risk); before/after comparison metric in the Gate A report (what changed in test-script output between prior-ship behavior and this iteration). **Do NOT use single-gate protocol when:** schema changes (even additive new fields); new test scripts or modified test assertions; new dependencies introduced; new runtime code paths (even prompt-reachable ones); multi-file coordinated edits across agents with unclear blast radius. **Example (session 2026-04-23):** W6.5 Planner subject-stance tuning + conditional Critic check. Two prompt files edited; existing `test-planner.ts` and `test-coherence-critic.ts` validated behavior; no schema, code, or dependency changes; explicit spot-check validated the one conditional path not exercised by existing seeds. Single-gate appropriate. **Counter-example:** any future workstream that adds a new `CriticIssue` type to the enum taxonomy is NOT schema-additive-eligible (enum widening affects downstream consumers even if "additive" in a literal sense) and uses standard two-gate. The purpose of this rule is to avoid ceremony on genuinely low-risk prompt iterations without opening the door to sneaking in substantive changes under the "tuning" label. If in doubt, use two-gate.

**43. Strategic concerns in tactical work surface design errors.** When the operator (or a planning chat reviewing tactical work) raises a strategic-shaped doubt during execution-phase work, treat it as an architectural signal, not a tactical objection. Reframe and re-evaluate. Do not defend the original choice. The pattern: an LLM-agent or a tactical analysis is producing verdicts against an internal model of "correct" that the operator doesn't share. The fix is always upstream of where the symptom shows. Tactical defense of the original conclusion is the wrong move every time it has occurred so far. **Eighteen sightings supporting promotion to rule (5 from initial promotion + 2 added 2026-04-28 + 3 added 2026-04-29 + 8 added 2026-05-01 to 2026-05-03 [session 21]):** (1) **W6.5 (subject-stance commitment, 2026-04-23):** per-slot Critic was flagging continuity issues; operator pushback reframed as Planner needing to commit subject-stance per video. Critic-prompt-tuning would have been wrong; Planner-side commitment was the right surface. (2) **Q5 (library-inventory-at-Critic, 2026-04-24):** Critic was guessing structural-vs-slot scope; operator pushback ("maybe we should have chosen X then?") reframed as Critic needing inventory data injected. Original choice (b) was tactically defensible; option (a) was strategically right. (3) **W11 collapse (2026-04-27):** Critic was flagging subject_discontinuity at severity high; operator pushback ("the video wasn't bad, success looks different to me than to you") reframed as Critic threshold being miscalibrated relative to operator's actual quality criterion. A 5-7 day Director architecture rebuild collapsed to 2-3 days of Critic prompt iteration. (4) **W7 hook-mechanism load-bearing (2026-04-24):** Copywriter brief was treating hook as a dangling string; operator pushback during W7 Q&A reframed hook_mechanism as the load-bearing planner output. The brief restructured around it. (5) **W4/W3 Planner-doesn't-name-exercises (Phase 4 Part B initial design):** earliest version of Planner prompt named specific exercises; operator pushback reframed as "Planner commits to body focus only — Director picks actual clips, prevents CD-invented-exercise failure mode." The decision moved upstream. (6) **Polish Sprint pause (2026-04-28):** Polish Sprint Pillar 1 was mid-execution at c4 with charter rewrite + stance-conditional thresholds shipped, calibration seeds harness scripts ready to run; operator pushback ("higher up wants simple results right now") reframed as needing a parallel Simple Pipeline architecture for shippable videos this week, not Pillar 1 calibration completion. Tactically defending Pillar 1 progress would have been wrong; pausing at c4 and pivoting to Simple Pipeline was the right surface. Polish Sprint stays parked until Simple Pipeline ships and content cadence stabilizes. (7) **Simple Pipeline two-product fork (2026-04-28):** initial Simple Pipeline brief (drafted post-Polish-Sprint-pause) had single-product framing (slot_count parameterized 1-5 of the same product). Operator pushback during brief Q&A reframed as two distinct products: routine videos (slot_count 2-5, instructive overlay) and meme/vibe videos (slot_count 1, punchy overlay) — different creative intents, different ideal outputs, requiring different agent stages. Tactically forcing single-product unification would have been wrong; recognizing the product fork was the right surface. Brief redrafted with both products from same infrastructure. (8) **S8 chore "this isn't a clean export, halt" (2026-04-29):** When operator hand-pasted S8 v2 JSON into the repo and asked agent to commit it as a "resync to working n8n state," agent halted on diff scope exceeding the documented three-point divergence pattern. Original framing assumed the diff was bounded; the diff wasn't. Agent surfaced unexpected items (poll cadence change, Drive node restructure, missing retry, credential renames) before committing. Tactically defending the chore label as "clean up an open followup early" would have been wrong — the cleanup framing was dragging in scope that wasn't actually clean. Pausing to verify scope upstream of the commit was the right surface. Followup remains open; chat-paste itself was the wrong transport for large multi-line JSON. (9) **c10 first-run "concurrency was already 1, real cause is cleanup-induced retry burst" (2026-04-29):** When Upstash Redis hit its 1000 cmd/sec cap during Simple Pipeline c10 first-run, planning chat hypothesized "13 jobs in burst exceeded concurrency, lower concurrency." Agent re-read the journal carefully, found that concurrency was already 1 from c7 (5b16e56), and surfaced the real cause: cleanup script deleting Postgres rows mid-flight while BullMQ jobs were still queued, causing worker to retry-burst through orphan jobs at near-CPU-speed. Tactically defending the "lower concurrency" recommendation would have been wrong; my proposed fix was a no-op against current code. Agent's pushback prevented shipping a false-fix. Real fix: defensive 500 cmd/sec limiter + operational discipline note (drain BullMQ before deleting Postgres rows). (10) **v1.1 iteration 2 wrap algorithm "Q4 spec gap, 2-line cap was wrong shape" (2026-04-29):** When operator review of v1.1 first iteration revealed iter-extreme (132-char overlay) overflowing despite scale-down to 30px floor, planning chat surfaced: this is a Q4 spec gap, not an implementation bug. The kickoff specced "2 lines max + scale-down" but reality required N-line wrap up to 5 lines. Tactically defending the 2-line + scale-down shape would have been wrong; extending to N-line wrap was the right design. Operator's "no, doesn't fit" was strategic-shaped feedback against my tactical iteration plan — a single-word rejection that surfaced a brief-Q&A spec gap. (11) **Brief filename suffix mismatch (session 21, 2026-05-01):** Editor agent v1.2 kickoff referenced `docs/briefs/editor-agent.md` but actual file was `docs/briefs/editor-agent-brief.md`. md5 verification on the real file succeeded; lineage was clean. Agent surfaced the mismatch as a halt-and-report rather than ploughing through. Same pattern recurred for v1.2.1 brief and session 21 docs brief. Tactically defending "the brief is missing" would have been wrong; the path was a typo precedent and the file was findable. Filed as a class — `brief-drafting-no-new-files-constraint-needs-scope` followup captures the broader pattern. (12) **HEAD SHA referenced from session memory rather than live git (session 21, 2026-05-01):** kickoff "HEAD = 9845684 (last c5.9 commit)" was stale by the time agent started — c6 Gate A artifact had landed at `4731fa6`. Agent surfaced lineage check (4731fa6 is descendant of 9845684; only docs/diagnostics added; no code drift) and proposed three options. Tactically resetting to 9845684 would have thrown away the artifact for cosmetic SHA-match; proceeding from 4731fa6 was correct. Pattern: kickoff-stated SHAs may lag real git state when authored hours apart from execution. (13) **Chat-paste markdown autolink corruption on JSON / `s.id` / `Date.now` (session 21, recurring):** every brief-or-diff paste through chat that contained URL-shaped tokens or square-bracket-paren patterns silently corrupted on transit. v1.2 brief, v1.2.1 brief, c6 cleanup attempts, S8 v2 JSON paste — all hit the pattern. File transport via `Write` tool works around. Filed as `chat-paste-markdown-autolink-corruption-pattern` followup. (14) **Upstash burst cap misdiagnosis loop ×3 (session 21, 2026-05-01):** three c6 attempts triggered `ERR max requests limit exceeded — Limit: 500000, Usage: 500001`. Agent (and planning chat) variously hypothesized: monthly cap, per-second cap, BullMQ stale connection. Real cause: third internal cap mechanism in Upstash that fires on tight-loop enqueue bursts, NOT visible in dashboard. Workaround: pace bulk enqueues with 2s sleep, OR drive enqueues through n8n S1's 30s polling cadence. Promoted to Rule 44. (15) **PING-only health probe insufficient for BullMQ worker state (session 21, 2026-05-01):** between c6 attempts, operator did `redis-cli PING` to verify Upstash health and reported "PONG, all good." Single-op probes pass even when bulk-op state is stuck. Agent's diagnostic that subsequently used `getJobCounts()` on all 5 queues (multi-op probe) surfaced the actual stuck state. Pattern: probe granularity must match the diagnostic question. Diagnostic-shape mismatch between PING (one-op) and BullMQ (many-op) is what allowed the misdiagnosis to persist. (16) **Cap interpretation as Upstash-side when actually application-layer (session 21, 2026-05-01):** Rule 43 sighting #14's reframe surfaced a deeper one — the cap mechanism existed at the Upstash side but the trigger was application-side (tight enqueue loop). Pre-reframe diagnostic was looking at the wrong layer. Pattern echoes sighting #9 (operational issue framed as code issue) and reinforces the principle. (17) **Mechanical bar passing while operator quality bar fails (session 21, v1.2 review, 2026-05-01):** Editor v1.2 Gate A: 6/6 reach human_qa, all clamps held, zero fallbacks across 19 segments. Mechanical bar reading PASS. Operator visual review: 2/6 solid, 1/6 good-but-long, 3/6 below uploadable. The mechanical bar didn't catch the keyframe-snap-padding bug that was discarding Editor's refinements; mechanical bar measured "did the pipeline complete" not "did the pipeline complete correctly." Surfaced the load-bearing-but-silent class of bug — refined boundaries computed correctly, persisted to `editor_outcome` payload correctly, never reached ffmpeg's render output because of the input-seek + `-c copy` bug. Promoted to Rule 47. (18) **Render-path bug existed since v1.0 but only became visible at v1.2.1's aggressive trim (session 21, 2026-05-02):** `buildTrimCommand`'s `-ss` placement before `-i` with `-c copy` shipped in v1.0 and was silent through v1.0/v1.1/v1.2 because Editor wasn't yet shipping refined boundaries — Pass A was always trimming at original `start_s`/`end_s` boundaries which happened to be keyframe-aligned by accident (segment boundaries from ingest were at scene cuts which are GOP starts). v1.2.1's aggressive Editor trim produced refined timestamps at arbitrary positions; keyframe-snap suddenly became visible as 5-12s padding overshoot. Promoted to Rule 46. The class — "load-bearing-but-silent code path that becomes load-bearing-and-broken when upstream behavior changes" — is the broader pattern. **Operator signals indicating Rule 43 application:** "maybe we should have chosen X then?" (explicit re-evaluation request); "success looks different to me than to you" (frame disagreement, not tactical objection); "this might be our downfall" (strategic concern, not blocker to manage); "I don't think we should change our plan that much" (strategic preservation, may indicate the proposed change misframes the problem). **What to do when Rule 43 fires:** (1) Pause the tactical work in progress (do not draft the brief, do not run the fix, do not commit); (2) Restate the operator's reframe in your own words to confirm you've understood it; (3) Trace the symptom-to-cause chain: where in the upstream pipeline did the wrong frame originate?; (4) Reframe the workstream from the upstream surface, not the symptom surface; (5) If the reframe collapses the original workstream entirely (as W11 did), say so explicitly; don't keep the workstream alive for sunk-cost reasons. **Observation (2026-04-28, updated 2026-04-29): Rule 43 reframe-cost compounds across nested sightings.** Sightings 6, 7 (session 19) and 8, 9, 10 (session 20) caught at varying execution depths with corresponding reframe costs: Sighting 6 (Polish Sprint pause) — caught mid-execution after c1-c4 commits landed on feat branch; reframe cost bounded — branch parks cleanly, work resumes mechanically. Sighting 7 (Simple Pipeline two-product fork) — caught during brief drafting via Q&A, before any tactical work committed; cheapest — Q&A quality compounds. Sighting 8 (S8 chore halt) — caught during chore execution, before commit; cheap — chore reverted, followup stays open. Sighting 9 (concurrency was already 1) — caught mid-c10 incident response, before false-fix shipped; moderate — required agent to push back against planning chat's proposed fix. Sighting 10 (Q4 spec gap, N-line wrap) — caught mid-v1.1 iteration, after 1 iteration cycle on the wrong shape; moderate — one wasted iteration, fixed in next. Pattern: Rule 43 application cost scales with how deep into execution the sighting fires. Catching at Q&A is cheapest. Catching at brief-drafting is cheap. Catching mid-execution is bounded-expensive (parked work or one wasted iteration). Catching after Gate A merged is most expensive (revert). Implication for brief Q&A practice: the Q&A itself is a Rule 43 prevention surface. Questions that surface architectural forks — "is this one product or two?" "is this a configuration knob or a separate workstream?" "is this 2 lines max or N lines?" "is this the canonical render input or a cost-optimization artifact?" — earn back hours of execution time when the answer surfaces a fork. Q&A quality compounds. Specifically for session 20: sighting 10 (Q4 spec gap on wrap algorithm) is the cleanest case of a Q&A miss producing one wasted iteration. The fix took ~30 minutes; the brief Q&A could have caught it with a question like "what's the maximum number of lines you'd accept before scaling font down?" — a single Q would have prevented the issue. Worth treating "what's the maximum N for this knob?" as a standard kickoff Q&A pattern for any algorithm with a configurable upper bound. **Observation specific to session 20: agent pushback against planning chat is itself a Rule 43 application.** Sighting 9 is a noteworthy variant: planning chat (me) was the entity producing the wrong-shape recommendation, and the agent was the entity applying Rule 43 by pushing back. The reframe pattern still holds — strategic concern (the diagnostic doesn't match the actual code state) surfaced during tactical work (the proposed concurrency fix), and tactical defense would have been wrong. But the directionality flipped from "operator → planning chat" to "agent → planning chat." This is healthy and should be encouraged: agents should push back when planning chat's recommendation contradicts the codebase reality, even when planning chat sounds confident. Worth naming explicitly in brief drafting going forward: agents are licensed to surface "this doesn't match what I see in the code" as a halt-and-report condition, not just to execute the plan as written. **Observation (2026-05-03, session 21): mechanical bar passing while operator quality bar fails is a Rule 43 prevention surface in its own right.** Sighting 17 surfaced the load-bearing-but-silent failure mode: every measurable diagnostic (Zod-validated outputs, Postgres state transitions, /health 200, queue counts clean, fallback-reasons empty) read PASS while the actual rendered output was buying broken trim from a bug that had shipped 8 weeks earlier. The fix surface (Rule 47) is to add a per-pipeline standalone verification of input-to-output adherence, not just intermediate state. For Editor specifically: a 1-segment standalone test that takes a refined bound and verifies the rendered duration matches within frame-rounding. The c1.2.1.5 + c1.2.1.6 standalone tests are the template — single segment, known refined bounds, ffprobe the output. Cost is ~30s of agent work + ~$0.005 of Gemini cost; would have caught the bug 6 hours earlier. Worth making this a standard kickoff Q&A pattern for any future render-path-touching workstream: "what's the in-isolation verification that proves the full input-to-output chain works?" — separate from "do the agents emit valid Zod-clean output?" **Observation (2026-05-03, session 21): three sightings about probe granularity (#15, #14, #16) reinforce the same pattern.** Sighting 15 (PING vs getJobCounts) is the cleanest case: single-op probe passes; multi-op probe surfaces the stuck state. Sighting 14 (Upstash burst cap) variant: cap mechanism is real but invisible in dashboard's two visible mechanisms (monthly, per-second). Sighting 16 closes the loop: the cap is Upstash-side, trigger is app-side. The unified pattern: when diagnosing a system with multiple state layers, the probe must touch the same layer as the failure mode. Diagnostic-shape mismatch is a reliable false-PASS source. For future work: when health checking, list the failure shapes you're checking AGAINST and pick a probe that touches each one. PING does not check BullMQ pool. Dashboard usage chart does not check burst-cap. Mechanical bar does not check render output. Each layer needs its own probe. **What NOT to do when Rule 43 fires:** defend the diagnostic evidence as if it were the conclusion (evidence can be technically correct and strategically wrong); argue the operator misunderstood the technical detail (operator probably grasped the technical detail and is signaling it's not the right axis); propose a hybrid that preserves both the original tactical choice and the operator's strategic concern (usually the wrong shape; pick one); keep the original brief in motion while the reframed work runs in parallel (creates ambiguity about which workstream is real). **Failure mode if Rule 43 is ignored:** the workstream ships to validation against a bar the operator doesn't share. Either Gate A blocks (wasted execution time), or Gate A passes but operator rejects at deploy (worse — code merged that solves the wrong problem). Both have happened in adjacent codebases. Avoiding both is why this rule is load-bearing.

**44. Upstash burst-rate limit fires on bulk job enqueues in tight loops, surfaces with the same error string as monthly-cap and per-second-cap limits but is a third internal mechanism not visible from the dashboard.** The error string `ERR max requests limit exceeded — Limit: 500000, Usage: 500001` is identical across three different cap mechanisms in Upstash: (1) monthly cap (visible in dashboard usage chart), (2) per-second cap (visible in dashboard rate panel), (3) burst cap on tight enqueue loops (NOT visible anywhere in dashboard). In session 21, three c6 attempts triggered the burst cap by enqueueing >6 jobs in <2s. Dashboard showed 1.7M / Unlimited usage, healthy. The error string matched the monthly-cap variant, leading to misdiagnosis as a plan-level issue when it was actually a tight-loop rate limit. **Workarounds:** pace bulk enqueues with explicit `await sleep(2000)` between jobs (see `src/scripts/run-editor-gate-a.ts` `SLEEP_BETWEEN_PAIRS_MS` constant); drive enqueues through n8n S1 cadence (30s polling naturally spaces them); don't trust application-layer rate limiters to defend against this — they cap job *processing* rate, not Redis *command* rate. Service hard-restart (`stop` + sleep 10s + `start`, NOT soft `restart`) often clears stale BullMQ connection pool state when this fires; soft `systemctl restart` reuses pool state and may not recover. See `docs/VPS-SERVERS.md` "Worker stuck-state and hard-restart procedure" for the diagnostic + recovery sequence.

**45. When n8n workflow JSON files in the repo describe routing logic that the live n8n instance does NOT have, the live instance silently routes everything through the older default path. The doc-vs-reality drift is real and recurring; trust live n8n state, not repo files.** In session 21, `n8n-workflows/S1-new-job.json` in repo had Pipeline-aware routing for Simple Pipeline, but the live n8n had the older S1 with `status: 'planning'` and `queue: 'planning'` hardcoded. Sheet-driven jobs with `Pipeline=simple` silently ran Phase 3.5 (advanced path) instead of Simple Pipeline. Surfaced via `template_id='phase3-parameterized-v1'` on nordpilates jobs that should have been Simple Pipeline (`template_id=null`). When in doubt, query Postgres for actual job shape (`SELECT id, status, template_id FROM jobs WHERE brand_id=X ORDER BY created_at DESC`) rather than trusting the Sheet's `Pipeline` column to mean what it says. n8n write access is operator-only per Rule 11; agent cannot import the routing. **Common follow-on issue after import:** `$json.__queue` from the Route Decision node is stripped by HTTP Request nodes. Switch on Queue and downstream nodes must reference `$('Route Decision').item.json.__queue` (and other `__` fields) explicitly. The pattern echoes Rule 11 (Sheets is input/view, Supabase is truth): for n8n routing, repo JSON is intent, live n8n is truth. When session-end docs touch updates n8n-workflows/*.json files, doc the live state, not the intent.

**46. ffmpeg `-ss` placement before `-i` with `-c copy` is input-seek + stream-copy and snaps to keyframes. After `-i` is output-seek and is frame-accurate but with `-c copy` may drop streams that lack keyframes in the kept range. Re-encode (`-c:v libx264 -preset medium -crf 18`) is the canonical fix for frame-accurate trim on arbitrary timestamps.** Three behaviors to know: (1) `-ss before -i` + `-c copy`: input-seek, fast, keyframe-aligned — fast and lossless but produces 0-N seconds of unwanted leading footage when the requested start lies between keyframes; (2) `-ss after -i` + `-c copy`: output-seek + stream-copy — frame-accurate when refined start ≥ keyframe; **drops video stream entirely when start < first keyframe of kept range** (subsequent filtergraph references like `[0:v]` then fail with "matches no streams" and exit 234); (3) `-ss after -i` + re-encode (`-c:v libx264 -preset medium -crf 18`): output-seek + decode/re-encode — always frame-accurate, costs ~5-8s/segment at CRF 18 medium preset. For trim windows where the refined start may land anywhere (Editor's output is arbitrary timestamps within original bounds), only option (3) is reliable. Always pair output-seek with re-encode unless input is guaranteed to have keyframes at trim-start positions. The bug existed in video-factory's `buildTrimCommand` since v1.0 but only became visible at Editor v1.2.1's aggressive trim levels (per Rule 43 sighting #18). Fixed in c1.2.1.5 + c1.2.1.6 (session 21). Three callers benefited: Simple Pipeline render.ts Pass A (load-bearing), Phase 3 clip-prep.ts (silent benefit; advanced trim was usually keyframe-aligned by accident), test-pipeline.ts (test harness). **For future ffmpeg work involving trim:** never use `-c copy` with `-ss` AFTER `-i` unless inputs are guaranteed keyframe-aligned at trim-start; default to re-encode; verify with a single-segment standalone test that ffprobes the output and checks rendered_duration matches refined_duration within frame-rounding tolerance (see Rule 47).

**47. Briefs that ship prompt or schema changes should be evaluated against working downstream code. When the render path was broken, v1.2.1 prompt evaluation was invalid; we judged outputs that didn't reflect what Editor was deciding. Verify the full pipeline produces expected behavior on a known-good standalone before judging prompt quality on production renders.** In session 21, two consecutive Editor prompt iterations (v1.2 + v1.2.1) appeared to produce mediocre output because the render-path bug was discarding their refinements. The "prompt iteration going backward" signal was actually "fix-applied differently as the prompt got more aggressive" — keyframe-snap padding masked Editor v1.2's modest refinements but exposed v1.2.1's aggressive ones, making v1.2.1 look worse than v1.2 even though Editor was working harder. Cost: ~6 hours diagnosing the wrong layer. **Mitigation pattern (template):** future prompt-iteration sequences should include a per-segment standalone verification of refined-bounds-to-render-output adherence BEFORE judging visual output quality. Single segment, known refined bounds (e.g., `refined_start = original_start + 2.0` to force the closed-GOP edge case), ffprobe the output, confirm rendered duration matches refined within frame-rounding tolerance. The c1.2.1.5 + c1.2.1.6 standalone tests are the canonical example — `npx tsx scratch-verify-trim-fix.ts` style, single segment, single ffmpeg invocation, single ffprobe check. Cost is ~30s of agent work + ~$0.005 of Gemini cost (none in this case — it's just ffmpeg). Catches a Rule 43 sighting #17-class bug 6 hours earlier than the operator visual review can. **When this rule applies:** any workstream that touches a prompt that produces output consumed by downstream code (Editor → render, Match-Or-Match → Editor → render, Planner → Director → render). When this rule does NOT apply: prompt iterations whose output goes only to a downstream LLM agent (not to deterministic code), where the LLM's tolerance for prompt drift naturally absorbs minor downstream surprises.
